$(document).ready(function() {
    ...
    $.ajax({
        type: "GET",
        url: baseURL + "exam_managment/ExamService.php/exams",
        dataType: "json",
        beforeSend: function (xhr) {
            xhr.setRequestHeader('Authorization', usr_api_key);
        },
        success: function (resp) {
            for (i = 0; i < resp.exam.length; i++) {
                $("#exm_para2").append("<option value=\"" + resp.exam[i].exm_id + "\" >" + resp.exam[i].exm_name+" - "+resp.exam[i].exm_discription + "</option>");
            }
        },
        error: function (e) {
			...
			for(var key in excel_input) {
				if(excel_input.hasOwnProperty(key)) {
					...
					for(i=0;i<excelsheets.length;i++){
						...
						outputdata.stu_id =  inputsheet.stu_admission_number;
						outputdata.exm_id = exm_para2;
						outputdata.year = exm_para1;
						outputdata.result = inputsheet.result;
						outputdata.stu_exm_index_number = inputsheet.stu_exm_index_number;
						
						$.ajax({
							type: "POST",
							url: baseURL + "excelfeed/ExamResultService.php/result_register",
							dataType: "json",
							data : outputdata,
							beforeSend: function (xhr) {
								xhr.setRequestHeader('Authorization', usr_api_key);
